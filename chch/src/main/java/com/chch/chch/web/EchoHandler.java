...
public class EchoHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		System.out.println(senderId + " : " + userSessionsMap.get(senderId).getId());
		...
		if (StringUtils.isNotEmpty(msg)) {
			...
			if (strs !=null && strs.length == 3) {
				...
				if ("notice".equals(cmd)) {
					...
					String notice = strs[2];
					WebSocketSession recieverWriterSession = userSessionsMap.get(userId);
					recieverWriterSession.sendMessage(new TextMessage("notice:"+notice));
				}
			}
...
//		}
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		userSessionsMap.remove(session.getAttributes().get("id"));
