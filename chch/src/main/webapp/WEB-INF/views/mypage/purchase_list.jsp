...
$(document).ready(function() {
	...
	<div class="select_view"> <!-- 리스트 갤러리 버튼 -->
		<button onclick="list()">
		...
		<div class="board_list" id="list_Area">
			...
			<c:if test="${empty purchase_list }">
				...
				<div class="item">
					...
					<div class="product">
					제목 : ${deal.book_title }<br>저자 : ${deal.book_author }</div>
					...
					<div class="price"><fmt:formatNumber value="${deal.deal_price }" pattern="#,###" />원</div>
					<div class="delete"><input type="button" value="삭제" name="deal_no" class="del"
						onclick="location.href='purchase_list_Update.do?deal_no=${deal.deal_no}'"></div>
				</div>
		</c:forEach>
		</c:if>
			</div>
		...
		<div class="board_list" id="gallery_Area">
			 ...
			 <!-- <div class="board_list_body"> -->
				...
				<c:if test="${not empty purchase_list }">
					...
					<a href="#"> <img alt="책이미지" src="resources/images/${deal.book_image }" >
						<br>${deal.book_title } <br class="price"><fmt:formatNumber value="${deal.deal_price }" pattern="#,###" />원
					</a> 
