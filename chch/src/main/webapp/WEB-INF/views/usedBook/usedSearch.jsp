...
<body>
		...
		<c:if test="${not empty bookList }">
			<c:forEach var="book" items="${bookList }">
				<div class="listRowBook">
					<div class="listColumnBook book_title"
						onclick="selectSearch(${book.book_no}, '${book.book_title}')"
						style="cursor: pointer; font-weight: bold;">${book.book_title}</div>
					<div class="">${book.book_author }/${book.book_publisher} /
						${book.book_kind }</div>
				</div>
			</c:forEach>
		</c:if>
	...
	<div id="searchResultArea">
		...
		<input id="searchResult2" name="book_title" value="" type="text"
